...
**Supported Visual Studio Versions**: 2017+
...
A `Dockerfile` has been included in this project with the setup required to create an image that can build this extension. This requires docker on windows to be set to `Windows Containers` mode.



Building the docker image can be done by editing the `$VERSION` variables inside the `build-builder.ps1` script, and then running it.

## Building in the Docker Container

In order to build the extensison inside the docker image created above, using your CI tool of choice perform the following steps:

1. Git clone this repository
2. Cd into the root of the repository
3. Run the `build-extension.ps1` script
